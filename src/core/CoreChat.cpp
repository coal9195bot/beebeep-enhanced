...
Chat Core::createGroupChat( const User& u, const Group& g, bool broadcast_message )
{
  ...
  if( !u.isLocal() )
  {
    sHtmlMsg = tr( "%1 %2 has added you to the group chat %3." ).arg( IconManager::instance().toHtml( "group-add.png", "*G*" ), Bee::userNameToShow( u, true ), QString( "<b>%1</b>" ).arg( Bee::replaceHtmlSpecialCharacters( c.name() ) ) );
    c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, false ) );
  }
  ...
  if( !sHtmlMsg.isEmpty() )
    dispatchSystemMessage( ID_DEFAULT_CHAT, u.id(), sHtmlMsg, DispatchToChat, ChatMessage::System, false );
...
bool Core::changeGroupChat( const User& u, const Group& g )
{
  ...
  if( c.name() != g.name() )
  {
    if( u.isLocal() )
      ...
      sHtmlMsg = tr( "%1 %2 has changed the group name from %3 to %4." ).arg( IconManager::instance().toHtml( "group.png", "*G*" ), Bee::userNameToShow( u, true ), Bee::replaceHtmlSpecialCharacters( c.name() ), QString( "<b>%1</b>" ).arg( Bee::replaceHtmlSpecialCharacters( g.name() ) ) );
    c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, false ) );
    chat_changed = true;
  ...
  foreach( User old_user, group_old_members.toList() )
  {
    if( !old_user.isLocal() )
    {
      if( !group_new_members.has( old_user.id() ) )
      {
        if( u.isLocal() )
        {
          sHtmlMsg = tr( "%1 %2 will be informed of your changes." ).arg( IconManager::instance().toHtml( "group-remove.png", "*G*" ) ).arg( Bee::userNameToShow( old_user, true ) );
          c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, false ) );
          group_removed_members.set( old_user );
        }

        if( c.removeUser( old_user.id() ) )
          user_removed_string_list << Bee::userNameToShow( old_user, true );
      }
    }
  }
  ...
  if( user_removed_string_list.size() > 0 )
  {
    if( u.isLocal() )
      ...
      sHtmlMsg = tr( "%1 %2 has removed members: %3." ).arg( IconManager::instance().toHtml( "group-remove.png", "*G*" ), Bee::userNameToShow( u, true ), Bee::stringListToTextString( user_removed_string_list, true ) );
    c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, false ) );
  ...
  foreach( User u, group_new_members.toList() )
  {
    ...
    {
      if( c.addUser( u.id() ) )
        user_added_string_list << Bee::userNameToShow( u, true );
      user_string_list.append( Bee::userNameToShow( u, true ) );
    }
  }
  ...
  if( user_added_string_list.size() > 0 )
  {
    if( u.isLocal() )
      ...
      sHtmlMsg = tr( "%1 %2 has added members: %3." ).arg( IconManager::instance().toHtml( "group-add.png", "*G*" ), Bee::userNameToShow( u, true ), Bee::stringListToTextString( user_added_string_list, true ) );
    c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, false ) );
  }
...
bool Core::removeUserFromGroupChat( const User& u, const QString& chat_private_id )
{
  ...
  else
  {
    if( c.hasUser( u.id() ) )
    {
      QString sHtmlMsg = tr( "%1 %2 has left the group chat %3." ).arg( IconManager::instance().toHtml( "group-remove.png", "*G*" ), Bee::userNameToShow( u, true ), QString( "<b>%1</b>" ).arg( Bee::replaceHtmlSpecialCharacters( c.name() ) ) );
      c.removeUser( u.id() );
      c.setLastModifiedToNow();
      c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, true ) );
      ChatManager::instance().setChat( c );
      dispatchSystemMessage( ID_DEFAULT_CHAT, u.id(), sHtmlMsg, DispatchToChat, ChatMessage::System, false );
      emit chatChanged( c );
      return true;
...
bool Core::removeLocalUserFromGroupChatByOther( const User& other_user, const QString& chat_private_id )
{
  ...
  if( !c.isGroup() )
    return false;
  QString sHtmlMsg = tr( "%1 %2 wants to remove you from the group chat %3. If you agree please leave the group." ).arg( IconManager::instance().toHtml( "group-remove.png", "*G*" ), Bee::userNameToShow( other_user, true ), QString( "<b>%1</b>" ).arg( Bee::replaceHtmlSpecialCharacters( c.name() ) ) );
  c.addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( sHtmlMsg ), ChatMessage::System, true ) );
  ChatManager::instance().setChat( c );
  dispatchSystemMessage( ID_DEFAULT_CHAT, other_user.id(), sHtmlMsg, DispatchToChat, ChatMessage::System, false );
  emit chatChanged( c );
  return true;
}
...
bool Core::removeChat( VNumber chat_id, bool save_chat_messages )
{
  ...
  if( c.isGroup() )
  {
    dispatchSystemMessage( ID_DEFAULT_CHAT, ID_LOCAL_USER, tr( "%1 You have left group chat %2." )
                           ...
                           DispatchToChat, ChatMessage::System, true );
...
int Core::sendChatMessage( VNumber chat_id, const QString& msg, bool is_important, bool can_be_delayed, bool is_source_code )
{
  if( !isConnected() )
  {
    dispatchSystemMessage( chat_id, ID_LOCAL_USER, tr( "Unable to send the message: you are not connected." ), DispatchToChat, ChatMessage::Other, false );
    return 0;
  }

  if( chat_id == ID_DEFAULT_CHAT && !Settings::instance().chatWithAllUsersIsEnabled() )
  {
    dispatchSystemMessage( chat_id, ID_LOCAL_USER, tr( "Unable to send the message: this chat is disabled." ), DispatchToChat, ChatMessage::Other, false );
    return 0;
  }
  ...
  ChatMessage cm( ID_LOCAL_USER, m, ChatMessage::Chat, true );
    ...
    foreach( Connection *user_connection, m_connections )
    {
      if( user_connection->sendMessage( m ) )
        ...
        dispatchSystemMessage( ID_DEFAULT_CHAT, ID_LOCAL_USER, tr( "Unable to send the message to %1." )
                               ...
                               DispatchToChat, ChatMessage::UserInfo, false );
    }

    if( messages_sent == 0 )
      dispatchSystemMessage( chat_id, ID_LOCAL_USER, tr( "Nobody has received the message." ), DispatchToChat, ChatMessage::Other, false );
  }
  else
...
int Core::sendChatMessageWithReply( VNumber chat_id, const QString& msg, bool is_important, bool can_be_delayed, bool is_source_code, const QString& reply_to_sender, const QString& reply_to_text )
{
  if( !isConnected() )
  {
    dispatchSystemMessage( chat_id, ID_LOCAL_USER, tr( "Unable to send the message: you are not connected." ), DispatchToChat, ChatMessage::Other, false );
  ...
  if( !c.isValid() )
  {
    qWarning() << "Invalid chat Id in Core::sendChatMessageWithReply";
    return 0;
  }
  ...
  Message m = Protocol::instance().chatMessageWithReply( c, msg_to_send, reply_to_sender, reply_to_text );
  if( is_important )
    m.setImportant();
  ...
  ChatMessage cm( ID_LOCAL_USER, m, ChatMessage::Chat, true );
  cm.setReplyToSender( reply_to_sender );
  cm.setReplyToText( reply_to_text );
  dispatchToChat( cm, chat_id );
  ...
  foreach( User u, user_list.toList() )
  {
    ...
    if( !sendMessageToLocalNetwork( u, group_refuse_message ) )
      dispatchSystemMessage( group_chat.id(), ID_LOCAL_USER, tr( "%1 %2 cannot be informed that you have left the group." )
                             .arg( IconManager::instance().toHtml( "group-remove.png", "*G*" ) ).arg( Bee::userNameToShow( u, true ) ),
                             DispatchToChat, ChatMessage::System, false );
  }
}
...
void Core::sendBuzzToUser( VNumber user_id )
{
  ...
  {
    QString sys_msg = tr( "%1 You have buzzed %2." ).arg( IconManager::instance().toHtml( "bell.png", "*Z*" ), Bee::userNameToShow( u, true ) );
...
void Core::addChatHeader( Chat* p_chat )
{
  ...
  if( chat_has_history )
    p_chat->addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( "" ), ChatMessage::History, false ) );
  ...
  if( p_chat->isDefault() )
  {
    ...
    p_chat->addMessage( ChatMessage( ID_SYSTEM_MESSAGE, Protocol::instance().systemMessage( header_msg ), ChatMessage::System, false ) );
  ...
  else if( p_chat->isPrivate() )
  {
    ...
    header_msg = tr( "%1 Chat with %2." ).arg( IconManager::instance().toHtml( "chat-small.png", "*C*" ), u.name().isEmpty() ? tr( "Unknown %1" ).arg( p_chat->privateUserId() ) : Bee::userNameToShow( u, true ) );
  }
  else
  {
    QStringList user_string_list;
