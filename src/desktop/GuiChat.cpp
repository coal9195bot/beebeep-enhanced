 ...
 : QWidget( parent )
{
  ...
  // Reply preview widget (Coal/Clawdbot enhancement)
  m_replyToText = "";
  m_replyToSender = "";
  mp_frameReplyPreview = new QFrame( this );
  mp_frameReplyPreview->setObjectName( "ReplyPreviewFrame" );
  mp_frameReplyPreview->setFrameShape( QFrame::StyledPanel );
  mp_frameReplyPreview->setStyleSheet( "QFrame#ReplyPreviewFrame { background-color: #3a3a3c; border-left: 3px solid #007AFF; border-radius: 4px; padding: 4px; }" );
  
  QHBoxLayout* reply_layout = new QHBoxLayout( mp_frameReplyPreview );
  reply_layout->setContentsMargins( 8, 4, 4, 4 );
  reply_layout->setSpacing( 8 );
  
  mp_lblReplyPreview = new QLabel( mp_frameReplyPreview );
  mp_lblReplyPreview->setWordWrap( true );
  mp_lblReplyPreview->setStyleSheet( "color: #cdd6f4; font-size: 12px;" );
  reply_layout->addWidget( mp_lblReplyPreview, 1 );
  
  mp_btnCancelReply = new QPushButton( mp_frameReplyPreview );
  mp_btnCancelReply->setIcon( IconManager::instance().icon( "delete.png" ) );
  mp_btnCancelReply->setFlat( true );
  mp_btnCancelReply->setMaximumSize( 24, 24 );
  mp_btnCancelReply->setToolTip( tr( "Cancel reply" ) );
  connect( mp_btnCancelReply, SIGNAL( clicked() ), this, SLOT( clearReply() ) );
  reply_layout->addWidget( mp_btnCancelReply );
  
  // Insert reply preview above the message input
  mp_splitter->insertWidget( 1, mp_frameReplyPreview );
...
void GuiChat::customContextMenu( const QPoint& p )
{
  ...
  act->setEnabled( !mp_teChat->textCursor().selectedText().isEmpty() );
  act = mp_menuContext->addAction( IconManager::instance().icon( "chat.png" ), tr( "Reply to selected text" ), this, SLOT( replyToSelectedText() ) );
  act->setEnabled( !mp_teChat->textCursor().selectedText().isEmpty() );
...
void GuiChat::sendMessage()
{
  ...
  if( !m_replyToText.isEmpty() )
  {
    emit newMessageWithReply( m_chatId, msg, false, m_replyToSender, m_replyToText );
    clearReply();
  }
  else
  {
...
void GuiChat::checkAnchorClicked( const QUrl& url )
{
  ...
  {
    QUrlQuery query( url );
    QString sender = QUrl::fromPercentEncoding( query.queryItemValue( "sender" ).toUtf8() );
    QString text = QUrl::fromPercentEncoding( query.queryItemValue( "text" ).toUtf8() );
    
    // Strip any HTML tags from the text
    text = Bee::removeHtmlTags( text );
    ...
    if( display_text.length() > 100 )
      display_text = display_text.left( 100 ) + "...";
    
    // Update preview
    mp_lblReplyPreview->setText( QString( "<b>%1</b><br>%2" ).arg( sender.toHtmlEscaped(), display_text.toHtmlEscaped() ) );
    mp_frameReplyPreview->show();
...
QString GuiChat::chatMessageToText( const ChatMessage& cm )
{
  ...
  if( !cm.isFromSystem() )
  {
    ...
    int read_status = 0;
    if( u.isLocal() && cm.type() == ChatMessage::Chat )
    {
      Chat c = ChatManager::instance().chat( m_chatId );
      if( c.isValid() && !c.isDefault() )
      {
        // If no users in unread list, everyone has read -> double check
        // Otherwise, message is sent but not yet read -> single check
...
void GuiChat::refreshReadStatus()
{
  Chat c = ChatManager::instance().chat( m_chatId );
  if( c.isValid() )
    setChat( c );
...
void GuiChat::replyToSelectedText()
{
  QTextCursor tc = mp_teChat->textCursor();
  if( !tc.hasSelection() )
    return;
  
  // Get the selected text (strip HTML)
  QString selected_text = tc.selection().toHtml( "UTF-8" );
  selected_text.replace( "</p>", "</p>\n" );
  selected_text.replace( "<br>", "\n" );
  selected_text = Bee::removeHtmlTags( selected_text );
  selected_text = selected_text.trimmed();
  
  // Try to determine the sender from context
  // For now, use a generic label - could be enhanced to detect actual sender
  QString sender_name = tr( "Message" );
  
  // Store reply state
  m_replyToText = selected_text;
  m_replyToSender = sender_name;
  
  // Truncate display text if too long
  QString display_text = selected_text;
  if( display_text.length() > 100 )
    display_text = display_text.left( 100 ) + "...";
  
  // Update preview
  mp_lblReplyPreview->setText( QString( "<b>%1</b><br>%2" ).arg( sender_name, display_text.toHtmlEscaped() ) );
  mp_frameReplyPreview->show();
  
  mp_teMessage->setFocus();
}

void GuiChat::clearReply()
{
  m_replyToText = "";
  m_replyToSender = "";
  mp_frameReplyPreview->hide();
