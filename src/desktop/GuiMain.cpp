...
void GuiMain::settingsChanged( QAction* act )
{
  ...
  case 107:
    ...
    break;
  // Bubble appearance settings (Coal/Clawdbot enhancement)
  case 200: case 201: case 202: case 203:
    Settings::instance().setChatBubbleTheme( settings_data_id - 200 );
    refresh_chat = true;
    break;
  case 220:
    Settings::instance().setChatBubbleCompactPadding( act->isChecked() );
    refresh_chat = true;
    break;
  default:
...
void GuiMain::sendMessageWithReply( VNumber chat_id, const QString& msg, bool is_source_code, const QString& reply_to_sender, const QString& reply_to_text )
{
#ifdef BEEBEEP_DEBUG
  int num_messages = beeCore->sendChatMessageWithReply( chat_id, msg, false, false, is_source_code, reply_to_sender, reply_to_text );
  qDebug() << num_messages << "messages sent (with reply)";
#else
...
void GuiMain::showChatSettingsMenu()
{
  ...
  act->setData( 9 );
  mp_menuChat->addSeparator();
  // === Bubble Appearance (Coal/Clawdbot enhancement) ===
  mp_menuChat->addSeparator();
  
  // Bubble Theme submenu
  QMenu* themeMenu = mp_menuChat->addMenu( tr( "Bubble theme" ) );
  QActionGroup* themeGroup = new QActionGroup( themeMenu );
  themeGroup->setExclusive( true );
  QStringList themeNames;
  themeNames << tr( "iMessage" ) << tr( "WhatsApp" ) << tr( "Telegram" ) << tr( "Classic (no bubbles)" );
  for( int i = 0; i < themeNames.size(); i++ )
  {
    act = themeMenu->addAction( themeNames.at( i ), this, SLOT( settingsChanged() ) );
    act->setCheckable( true );
    act->setChecked( Settings::instance().chatBubbleTheme() == i );
    act->setData( 200 + i );
    themeGroup->addAction( act );
  }
  
  // Compact bubble padding toggle
  act = mp_menuChat->addAction( tr( "Compact bubble padding" ), this, SLOT( settingsChanged() ) );
  act->setCheckable( true );
  act->setChecked( Settings::instance().chatBubbleCompactPadding() );
  act->setData( 220 );
